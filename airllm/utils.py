...
def split_and_save_layers(checkpoint_path, layer_shards_saving_path=None, splitted_model_dir_name='splitted_model',
    ...
    if layer_names is None:
        # layout classico: model.layers.{i}.*
        n_layers = len(
            set(
                int(k.split(".")[2])
                for k in index.keys()
                if "model.layers" in k and k.split(".")[2].isdigit()
            )
        )
    else:
        # layout generico: può essere model.layers.{i}.* o model.layers.layers.{i}.*
        ...
        for k in index.keys():
            ...
            # parte della chiave dopo il prefix (es: ".0.self_attn..." o ".layers.0.self_attn...")
            suffix = k.split(prefix, 1)[1]
            parts = suffix.split(".")
            ...
            if len(parts) > 1 and parts[1].isdigit():
                ...
                # fallback: cerca la prima parte numerica dopo il prefix
                for p in parts:
                    if p.isdigit():
                        idx = int(p)
                        break
    ...
    for layer in tqdm(layers):
        ...
        if len(shards) > 0:
            ...
            # caso: modello non shardato (singolo file) per questo layer
            shards = [v for k, v in index.items() if k.startswith(layer)]
            if len(shards) == 0:
                # nessun peso con questo prefix: capita con alcuni layout Qwen3.5/3.6,
                # in quel caso saltiamo questo layer e passiamo al successivo.
                continue
