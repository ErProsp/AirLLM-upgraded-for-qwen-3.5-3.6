...
class AirLLMBaseModel(GenerationMixin):
    ...
    def init_model(self):
        ...
        # 1) Prova BetterTransformer solo se disponibile e compatibile
        if self.get_use_better_transformer() and _bettertransformer_available and BetterTransformer is not None:
            try:
                ...
                # API di Optimum: BetterTransformer.transform(model) [web:58][web:79][web:82][web:78]
                self.model = BetterTransformer.transform(base_model)
            except Exception:
                # qualsiasi errore (versioni non compatibili, deprecazioni, ecc.)
                del base_model
                clean_memory()
                self.model = None

        # 2) Se BetterTransformer non è usabile, prova attn_implementation="sdpa"
        if self.model is None:
            try:
                print(
                    "new version of transformer, no need to use BetterTransformer, "
        ...
        # 3) Fallback finale: modello “plain”
        if self.model is None:
            print(
                "neither BetterTransformer nor attn_implementation='sdpa' is available, "
        ...
        # Da qui in poi lascia identico all’originale:
    ...
    def get_past_key_values_cache_seq_len(self, past_key_values):
        # transformers recenti usano Cache/DynamicCache per il KV cache [web:115][web:112]
        if cache_utils_installed:
            ...
            if Cache is not None and isinstance(past_key_values, Cache):
                # DynamicCache espone get_seq_length() per ottenere la lunghezza della cache [web:115][web:114]
                try:
                    return past_key_values.get_seq_length()
                except Exception:
                    # fallback: usa la shape del primo key_cache se presente
                    try:
        ...
        # Formato legacy: lista/tupla di layer -> (k, v) con shape [..., seq_len, ...]
    ...
    def forward(
        ...
        if cache_utils_installed:
            # Non supportiamo ancora DynamicCache: disattiva completamente il KV cache. [web:115][web:114]
            use_cache = False
